<template>
  <!-- PRODUTOS -->
  <div class="produtos pt-32 pb-64">
    <div class="container">
      <h3>Produtos</h3>
      <div
        class="lista-produtos display-flex align-items-center flex-wrap-wrap gap-16 pt-16"
      >
        <div class="produto">
          <img
            src="https://fakeimg.pl/284x284/060707/00fbdb?text=IMAGEM+AQUI&font=bebas"
            alt="Categoria X"
          />
          <div class="detalhes">
            <p class="nome-produto">Título do Produto</p>
            <p class="categoria">Categoria</p>
            <p class="preco">R$ 1929,59</p>
            <button class="no-carrinho">Adicionado</button>
          </div>
        </div>

        <div class="produto">
          <img
            src="https://fakeimg.pl/284x284/060707/00fbdb?text=IMAGEM+AQUI&font=bebas"
            alt="Categoria X"
          />
          <div class="detalhes">
            <p class="nome-produto">Título do Produto</p>
            <p class="categoria">Categoria</p>
            <p class="preco">R$ 1929,59</p>
            <button class="carrinho">Adicionar ao Carrinho</button>
          </div>
        </div>
      </div>
    </div>
  </div>
  <!-- FIM DOS PRODUTOS -->
</template>

<script>
export default {};
</script>

<style lang="scss" scoped></style>
