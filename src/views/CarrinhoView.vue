<template>
  <div>
    <header class="header">
      <div class="container">
        <div
          class="display-flex justify-content-space-between align-items-center pt-16 pb-16"
        >
          <a class="logo" href="/">
            <img src="../assets/images/loja.png" alt="" />
          </a>
          <div class="opcoes display-flex align-items-center gap-32">
            <div class="display-flex align-items-center gap-8">
              <svg
                version="1.1"
                xmlns="http://www.w3.org/2000/svg"
                xmlns:xlink="http://www.w3.org/1999/xlink"
                x="0px"
                y="0px"
                viewBox="0 0 512 512"
                style="enable-background: new 0 0 512 512"
                xml:space="preserve"
              >
                <g>
                  <g>
                    <path
                      d="M256,0C114.841,0,0,114.841,0,256s114.841,256,256,256s256-114.841,256-256S397.159,0,256,0z M346.895,456.06c-27.716,12.642-58.497,19.695-90.895,19.695c-32.398,0-63.18-7.053-90.895-19.695v-88.595c0-24.969,20.313-45.281,45.281-45.281h91.228c24.968,0,45.281,20.313,45.281,45.281V456.06z M383.14,435.149v-67.684c0-44.953-36.572-81.526-81.526-81.526h-4.533c32.021-15.369,54.184-48.113,54.184-85.94c0-52.53-42.736-95.265-95.266-95.265c-52.53,0-95.266,42.736-95.266,95.265c0,37.828,22.164,70.571,54.184,85.94h-4.533c-44.953,0-81.526,36.572-81.526,81.526v67.684C72.847,395.282,36.245,329.834,36.245,256c0-121.173,98.582-219.755,219.755-219.755S475.755,134.827,475.755,256C475.755,329.834,439.153,395.282,383.14,435.149z M256,259.019c-32.544,0-59.021-26.477-59.021-59.021c0-32.544,26.477-59.02,59.021-59.02s59.021,26.477,59.021,59.02C315.021,232.544,288.544,259.019,256,259.019z"
                    />
                  </g>
                </g>
              </svg>
              <a href="/">Entrar/Cadastrar</a>
            </div>
            <div class="display-flex align-items-center gap-8">
              <svg
                version="1.1"
                xmlns="http://www.w3.org/2000/svg"
                xmlns:xlink="http://www.w3.org/1999/xlink"
                x="0px"
                y="0px"
                viewBox="0 0 512 512"
                style="enable-background: new 0 0 512 512"
                xml:space="preserve"
              >
                <g>
                  <g>
                    <path
                      d="M386.689,304.403c-35.587,0-64.538,28.951-64.538,64.538s28.951,64.538,64.538,64.538c35.593,0,64.538-28.951,64.538-64.538S422.276,304.403,386.689,304.403z M386.689,401.21c-17.796,0-32.269-14.473-32.269-32.269c0-17.796,14.473-32.269,32.269-32.269c17.796,0,32.269,14.473,32.269,32.269C418.958,386.738,404.485,401.21,386.689,401.21z"
                    />
                  </g>
                </g>
                <g>
                  <g>
                    <path
                      d="M166.185,304.403c-35.587,0-64.538,28.951-64.538,64.538s28.951,64.538,64.538,64.538s64.538-28.951,64.538-64.538S201.772,304.403,166.185,304.403z M166.185,401.21c-17.796,0-32.269-14.473-32.269-32.269c0-17.796,14.473-32.269,32.269-32.269c17.791,0,32.269,14.473,32.269,32.269C198.454,386.738,183.981,401.21,166.185,401.21z"
                    />
                  </g>
                </g>
                <g>
                  <g>
                    <path
                      d="M430.15,119.675c-2.743-5.448-8.32-8.885-14.419-8.885h-84.975v32.269h75.025l43.934,87.384l28.838-14.5L430.15,119.675z"
                    />
                  </g>
                </g>
                <g>
                  <g>
                    <rect
                      x="216.202"
                      y="353.345"
                      width="122.084"
                      height="32.269"
                    />
                  </g>
                </g>
                <g>
                  <g>
                    <path
                      d="M117.781,353.345H61.849c-8.912,0-16.134,7.223-16.134,16.134c0,8.912,7.223,16.134,16.134,16.134h55.933c8.912,0,16.134-7.223,16.134-16.134C133.916,360.567,126.693,353.345,117.781,353.345z"
                    />
                  </g>
                </g>
                <g>
                  <g>
                    <path
                      d="M508.612,254.709l-31.736-40.874c-3.049-3.937-7.755-6.239-12.741-6.239H346.891V94.655c0-8.912-7.223-16.134-16.134-16.134H61.849c-8.912,0-16.134,7.223-16.134,16.134s7.223,16.134,16.134,16.134h252.773v112.941c0,8.912,7.223,16.134,16.134,16.134h125.478l23.497,30.268v83.211h-44.639c-8.912,0-16.134,7.223-16.134,16.134c0,8.912,7.223,16.134,16.134,16.134h60.773c8.912,0,16.134-7.223,16.135-16.134V264.605C512,261.023,510.806,257.538,508.612,254.709z"
                    />
                  </g>
                </g>
                <g>
                  <g>
                    <path
                      d="M116.706,271.597H42.487c-8.912,0-16.134,7.223-16.134,16.134c0,8.912,7.223,16.134,16.134,16.134h74.218c8.912,0,16.134-7.223,16.134-16.134C132.84,278.82,125.617,271.597,116.706,271.597z"
                    />
                  </g>
                </g>
                <g>
                  <g>
                    <path
                      d="M153.815,208.134H16.134C7.223,208.134,0,215.357,0,224.269s7.223,16.134,16.134,16.134h137.681c8.912,0,16.134-7.223,16.134-16.134S162.727,208.134,153.815,208.134z"
                    />
                  </g>
                </g>
                <g>
                  <g>
                    <path
                      d="M180.168,144.672H42.487c-8.912,0-16.134,7.223-16.134,16.134c0,8.912,7.223,16.134,16.134,16.134h137.681c8.912,0,16.134-7.223,16.134-16.134C196.303,151.895,189.08,144.672,180.168,144.672z"
                    />
                  </g>
                </g>
              </svg>
              <a href="/">Rastrear Pedido</a>
            </div>
            <!-- CARRINHO -->
            <div class="carrinho display-flex align-items-center gap-8">
              <svg
                version="1.1"
                height="34"
                width="34"
                xmlns="http://www.w3.org/2000/svg"
                xmlns:xlink="http://www.w3.org/1999/xlink"
                x="0px"
                y="0px"
                viewBox="0 0 511.997 511.997"
                style="enable-background: new 0 0 511.997 511.997"
                xml:space="preserve"
              >
                <g>
                  <g>
                    <path
                      d="M405.387,362.612c-35.202,0-63.84,28.639-63.84,63.84s28.639,63.84,63.84,63.84s63.84-28.639,63.84-63.84S440.588,362.612,405.387,362.612z M405.387,451.988c-14.083,0-25.536-11.453-25.536-25.536s11.453-25.536,25.536-25.536c14.083,0,25.536,11.453,25.536,25.536S419.47,451.988,405.387,451.988z"
                    />
                  </g>
                </g>
                <g>
                  <g>
                    <path
                      d="M507.927,115.875c-3.626-4.641-9.187-7.348-15.079-7.348H118.22l-17.237-72.12c-2.062-8.618-9.768-14.702-18.629-14.702H19.152C8.574,21.704,0,30.278,0,40.856s8.574,19.152,19.152,19.152h48.085l62.244,260.443c2.062,8.625,9.768,14.702,18.629,14.702h298.135c8.804,0,16.477-6.001,18.59-14.543l46.604-188.329C512.849,126.562,511.553,120.516,507.927,115.875z M431.261,296.85H163.227l-35.853-150.019h341.003L431.261,296.85z"
                    />
                  </g>
                </g>
                <g>
                  <g>
                    <path
                      d="M173.646,362.612c-35.202,0-63.84,28.639-63.84,63.84s28.639,63.84,63.84,63.84s63.84-28.639,63.84-63.84S208.847,362.612,173.646,362.612z M173.646,451.988c-14.083,0-25.536-11.453-25.536-25.536s11.453-25.536,25.536-25.536s25.536,11.453,25.536,25.536S187.729,451.988,173.646,451.988z"
                    />
                  </g>
                </g>
              </svg>
              <a href="#">Carrinho</a>
            </div>
          </div>
        </div>
      </div>
    </header>

    <!-- MENU NAVEGAÇÃO -->
    <nav>
      <div
        class="container display-flex align-items-center justify-content-space-between pt-16 pb-16"
      >
        <a href="/">Categoria X</a>
        <a href="/">Categoria Y</a>
        <a href="/">Categoria Z</a>
        <a href="/">Categoria W</a>
        <a href="/">Categoria K</a>
      </div>
    </nav>
    <!-- FIM DA NAVEGAÇÃO -->

    <div class="produtos-carrinho pt-64 pb-32">
      <div class="container">
        <h3>Carrinho de Compras</h3>
        <div class="lista-produtos-carrinho mt-32">
          <div
            class="produto-carrinho display-flex justify-content-space-between align-items-center gap-16 pb-24"
          >
            <div class="detalhes display-flex align-items-center gap-16">
              <figure>
                <img
                  src="https://fakeimg.pl/180x180/060707/00fbdb?text=IMAGEM+AQUI&font=bebas"
                  alt="Categoria X"
                />
              </figure>
              <article>
                <p class="nome-produto">Título do Produto</p>
                <p class="categoria">Categoria</p>
                <p class="preco">R$ 1929,59</p>
              </article>
            </div>
            <button>
              <!-- Uploaded to: SVG Repo, www.svgrepo.com, Generator: SVG Repo Mixer Tools -->
              <svg
                width="24px"
                height="24px"
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M10 10V16M14 10V16M18 6V18C18 19.1046 17.1046 20 16 20H8C6.89543 20 6 19.1046 6 18V6M4 6H20M15 6V5C15 3.89543 14.1046 3 13 3H11C9.89543 3 9 3.89543 9 5V6"
                  stroke="#ffffff"
                  stroke-width="1.5"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                />
              </svg>
              Remover do carrinho
            </button>
          </div>

          <div
            class="produto-carrinho display-flex justify-content-space-between align-items-center gap-16 pb-24"
          >
            <div class="detalhes display-flex align-items-center gap-16">
              <figure>
                <img
                  src="https://fakeimg.pl/180x180/060707/00fbdb?text=IMAGEM+AQUI&font=bebas"
                  alt="Categoria X"
                />
              </figure>
              <article>
                <p class="nome-produto">Título do Produto</p>
                <p class="categoria">Categoria</p>
                <p class="preco">R$ 1929,59</p>
              </article>
            </div>
            <button>
              <!-- Uploaded to: SVG Repo, www.svgrepo.com, Generator: SVG Repo Mixer Tools -->
              <svg
                width="24px"
                height="24px"
                viewBox="0 0 24 24"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
              >
                <path
                  d="M10 10V16M14 10V16M18 6V18C18 19.1046 17.1046 20 16 20H8C6.89543 20 6 19.1046 6 18V6M4 6H20M15 6V5C15 3.89543 14.1046 3 13 3H11C9.89543 3 9 3.89543 9 5V6"
                  stroke="#ffffff"
                  stroke-width="1.5"
                  stroke-linecap="round"
                  stroke-linejoin="round"
                />
              </svg>
              Remover do carrinho
            </button>
          </div>

          <p class="total">Total: R$ 1929,59</p>
        </div>
      </div>
    </div>

    <!--
########
RODAPÉ
########
-->

    <!-- BANNER RODAPÉ -->
    <div class="banner-rodape pb-64">
      <div class="container">
        <img src="../assets/images/banner-rodape.webp" alt="Banner Rodapé" />
      </div>
    </div>

    <!-- NEWSLETTER -->
    <div class="newsletter pt-32 pb-32">
      <div class="container">
        <div
          class="display-flex justify-content-space-between align-items-center"
        >
          <div class="titulo">
            <p>CADASTRE-SE EM NOSSA NEWSLETTER</p>
            <p>e receba novidades e promoções</p>
          </div>
          <div class="form display-flex align-items-center gap-8">
            <input type="text" placeholder="Digite seu nome" />
            <input type="email" placeholder="Digite seu email" />
            <button>Inscreva-se</button>
          </div>
        </div>
      </div>
    </div>

    <footer class="pt-32 pb-32">
      <div class="container">
        <div class="display-flex justify-content-space-between gap-32">
          <div class="logo">
            <img src="../assets/images/loja-rodape.png" alt="" />
          </div>
          <div class="sobre">
            <h4>Sobre</h4>
            <p>
              Este projeto foi desenvolvido por os alunos do 4º perído de
              Sistemas de Informação na Disciplina de Frameworks Frontend,
              utilizando Vue.JS 3
            </p>
          </div>
          <div class="cartoes">
            <h4>Formas de Pagamento</h4>
            <img src="../assets/images/formas-cartoes.png" />
          </div>
        </div>
      </div>
    </footer>

    <div class="copyright text-align-center pt-16 pb-16">
      <div class="container">
        <small
          ><strong>Desenvolvido por:</strong> Carlos Rangel, Rangel Xavier e
          Carlos Rangel Xavier - Uninassau - IV Período de Sistemas de
          Informação</small
        >
      </div>
    </div>
  </div>
</template>

<script>
export default {};
</script>

<style lang="scss" scoped></style>
